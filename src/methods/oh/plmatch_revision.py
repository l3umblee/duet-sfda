...
def cal_acc_ece(loader, netF, netB, netC, cfg, flag=False):
    start_test = True
    with torch.no_grad():
        iter_test = iter(loader)
        for i in range(len(loader)):
    ...
    classic_ece(all_output_pred, all_label, cfg.MODEL.METHOD, cfg.output_dir)
    _, predict = torch.max(all_output, 1)
    accuracy = torch.sum(torch.squeeze(predict).float() == all_label).item() / float(all_label.size()[0])
    mean_ent = torch.mean(loss.Entropy(nn.Softmax(dim=1)(all_output))).cpu().data.item()
...
def train_target(cfg):
    ...
    if cfg.SETTING.DATASET == 'VISDA-C':
        acc_s_te, acc_list = cal_acc_ece(dset_loaders['test'], netF, netB, netC, cfg,True)
        # log_str = ('Task: {}, Iter:{}/{}; Cycle: {}/{}; '
        #            'Accuracy = {:.2f}%').format(cfg.name, iter_num, max_iter,
        #                                                               curr_cycle + 1, cfg.ACTIVE.CYCLE,
        #                                                               acc_s_te) + '\n' + acc_list
    else:
        acc_s_te, _ = cal_acc_ece(dset_loaders['test'], netF, netB, netC, cfg,False)
        # log_str = ('Task: {}, Iter:{}/{}; Cycle: {}/{}; '
        #            'Accuracy = {:.2f}%').format(cfg.name, iter_num, max_iter,
        #                                                               curr_cycle + 1, cfg.ACTIVE.CYCLE,
        #                                                               acc_s_te)
