...
class MetricProjection(nn.Module):
    def __init__(self, latent_dim=512, hidden_dim=64):
        super().__init__()

        self.D = latent_dim
        self.hidden_dim = hidden_dim

        metric_tensor = torch.randn(hidden_dim, hidden_dim)
    ...
    def forward(self, feas, prototypes):
        bs = feas.size(0)
        proj_feas = self.projection(feas)  # [B, HD]
        proj_prototypes = self.projection(prototypes)  # [B, HD]
        v = proj_prototypes - proj_feas
...
@torch.no_grad()
def k_nearest_metric(features, features_bank, probs_bank, proj_head, num_neighbors):
    pred_probs = []
    pred_probs_all = []
    ...
    for feats in features.split(64):
        metric_feas = proj_head.projection(feats)  # [64, 512]

        if start_test:
            all_metric_feas = metric_feas.detach().float().cpu()
            start_test = False
        else:
            all_metric_feas = torch.cat((all_metric_feas, metric_feas.float().cpu()), 0)

    for feats in all_metric_feas.split(64):
        v = feats.unsqueeze(1) - all_metric_feas.unsqueeze(0)
        metric_dist = torch.einsum('bni,ij,bnj->bn', v, metric_tensor, v)

        _, idxs = metric_dist.sort()
        idxs = idxs[:, :num_neighbors]
        probs = probs_bank[idxs, :].mean(1)
        pred_probs.append(probs)
...
def refine_predictions(
    ...
    pred_labels, probs, pred_labels_all, pred_labels_hard = k_nearest_metric(
        features, feature_bank, probs_bank, proj_head, num_neighbors
    )
...
@torch.no_grad()
def eval_and_label_dataset(epoch, model, proj_head, loader, cfg):
    ...
    pred_labels, _, _, _ = refine_predictions(features, probs, banks, proj_head, cfg.PLUE.NUM_NEIGHBORS)
...
def train(epoch, net, moco_model, proj_head, optimizer, trainloader, banks, cfg, CE):
    loss = 0
    acc = 0
    ...
    for batch_idx, batch in enumerate(trainloader):
        ...
        feats_s, logits_s = moco_model(strong_x, cls_only=True, dset=cfg.SETTING.DATASET)
        _, metric_loss = proj_head(feats_w, feats_s)

        if cfg.PLUE.LABEL_REFINEMENT:
            with torch.no_grad():
                probs_w = F.softmax(logits_w, dim=1)
                pseudo_labels_w, probs_w, _, _ = refine_predictions(feats_w, probs_w, banks, proj_head, cfg.PLUE.LABEL_REFINEMENT)
        else:
            probs_w = F.softmax(logits_w, dim=1)
        ...
        l = loss_cls + loss_ctr + loss_div + metric_loss
...
def train_target(cfg):
    ...
    latent_dim = netC.fc.weight.size(-1)
    ...
    optimizer = optim.SGD(param_group, lr=cfg.OPTIM.LR, weight_decay=5e-4)

    moco_model = AdaMoCo(src_model=base_model, momentum_model=momentun_model, features_length=cfg.bottleneck,
                         num_classes=cfg.class_num,
                         dataset_length=len(dset_loaders['target'].dataset), temporal_length=cfg.PLUE.TEMPORAL_LENGTH)
    CE = nn.CrossEntropyLoss(reduction='none')

    acc, banks, _, _ = eval_and_label_dataset(0, moco_model, proj_head, dset_loaders["target"], cfg)
    max_acc = 0
    best_epoch = 0
    ...
    for epoch in range(cfg.TEST.MAX_EPOCH + 1):
        ...
        train(epoch, base_model, moco_model, proj_head, optimizer, dset_loaders["target"], banks, cfg, CE)  # train net1
        torch.cuda.empty_cache()
        acc, banks, gt_labels, pred_labels = eval_and_label_dataset(epoch, moco_model, proj_head, dset_loaders["test"], cfg)
