...
def create_cifarc_dataset(
    ...
    print(y_test[3])
    image = Image.fromarray((x_test[3] * 255).astype(np.uint8))
    # image.show()
    image.save("a.jpg")
...
def create_imagenetc_dataset(
    ...
    corruptions_seq = corruptions_seq if "mixed_domains" in setting else [corruption]
    corruption_dir_path = os.path.join(data_dir,corruption_noise, corruptions_seq[0], str(severity))
