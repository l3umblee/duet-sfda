...
def get_transform(dataset_name, adaptation):
    ...
    if adaptation == "adacontrast":
        ...
        if dataset_name in {"cifar10", "cifar100", "cifar10_c", "cifar100_c"}:
            ...
            # note that ImageNet-C is already resized and centre cropped
            transform = get_augmentation_versions(aug_versions="tws", aug_type="moco-v2-light", res_size=224,
                                                  crop_size=224)
        elif dataset_name in {"domainnet126"}:
            transform = get_augmentation_versions(aug_versions="tws", aug_type="moco-v2", res_size=256, crop_size=224)
        else:
            # use classical ImageNet transformation procedure
            transform = get_augmentation_versions_image(aug_versions="imnn", aug_type="moco-v2", res_size=256,
                                                        crop_size=224)
