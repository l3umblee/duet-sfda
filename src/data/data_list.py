...
class SynthImageList(Dataset):
    def __init__(self, image_list, synth_list, labels=None, synth_labels=None, transform=None, target_transform=None, mode='RGB'):
        self.image_list = image_list
        self.synth_list = synth_list

        imgs = make_dataset(image_list, labels)
        if synth_list is not None:
            synth_imgs = make_dataset(synth_list, synth_labels)
            self.combined_imgs = imgs + synth_imgs
        else:
            synth_imgs = None
            self.combined_imgs = imgs
    ...
    def __getitem__(self, index):
        path, target = self.combined_imgs[index]
        img = self.loader(path)
        if self.transform is not None:
            img = self.transform(img)
        if self.target_transform is not None:
        ...
        return img, target, index, mask

    def __len__(self):
        return len(self.combined_imgs)
